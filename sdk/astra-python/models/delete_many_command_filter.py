...
@dataclass
class DeleteManyCommand_filter(Parsable):
    """
    Filter clause based on which documents are identified
    """
    @staticmethod
    def create_from_discriminator_value(parse_node: Optional[ParseNode] = None) -> DeleteManyCommand_filter:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: DeleteManyCommand_filter
        """
        if not parse_node:
            raise TypeError("parse_node cannot be null.")
        return DeleteManyCommand_filter()
