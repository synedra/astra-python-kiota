...
if TYPE_CHECKING:
    from .delete_one_command_filter import DeleteOneCommand_filter
    from .json_node import JsonNode
    from .sort_clause import SortClause

@dataclass
class DeleteOneCommand(Parsable):
    """
    Command that finds a single document and deletes it from a collection
    """
    @staticmethod
    def create_from_discriminator_value(parse_node: Optional[ParseNode] = None) -> DeleteOneCommand:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: DeleteOneCommand
        """
        if not parse_node:
    ...
    def get_field_deserializers(self,) -> Dict[str, Callable[[ParseNode], None]]:
        ...
        fields: Dict[str, Callable[[Any], None]] = {
            "filter": lambda n : setattr(self, 'filter', n.get_object_value(DeleteOneCommand_filter)),
