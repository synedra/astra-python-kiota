...
@dataclass
class CountDocumentsCommands(Parsable):
    """
    Command that returns count of documents in a collection based on the collection.
    """
    @staticmethod
    def create_from_discriminator_value(parse_node: Optional[ParseNode] = None) -> CountDocumentsCommands:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: CountDocumentsCommands
        """
        if not parse_node:
            raise TypeError("parse_node cannot be null.")
        return CountDocumentsCommands()
